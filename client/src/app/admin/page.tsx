...
export default function AdminDashboard() {
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/users/${editingUser._id}`, formData);
  ...
  const toggleServiceStatus = async (id: string) => {
    try {
      ...
      const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/services/${id}`, { status: newStatus });
  ...
  const handleSaveService = async (e: any) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/services`, serviceFormData);
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-xl flex justify-between items-center">
                  ...
                  <button 
                    onClick={async () => { 
                      ...
                      try { await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/settings`, { maintenanceMode: newVal }); toast.success('Maintenance mode saved to DB!'); } catch(e) { toast.error('Failed to save'); }
                    }}
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-xl flex justify-between items-center">
                  ...
                  <button 
                    onClick={async () => { 
                      ...
                      try { await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/settings`, { aiAssistant: newVal }); toast.success('AI settings saved to DB!'); } catch(e) { toast.error('Failed to save'); }
                    }}
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-xl">
                  ...
                  <div className="flex gap-4">
                    ...
                    <button 
                      onClick={async () => {
                        try {
                          await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/settings`, { bookingCommission: systemSettings.bookingCommission });
