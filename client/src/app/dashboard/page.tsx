...
export default function Dashboard() {
  ...
  const downloadTicket = async (booking: any) => {
    ...
    const trainNum = booking.trainId?.trainNumber || booking.mockTrainId?.substring(0, 5) || 'N/A';
    const rawTrainName = booking.trainId?.name?.toUpperCase() || booking.serviceType?.toUpperCase() || 'N/A';
    const trainName = rawTrainName === 'TRAIN' ? 'KAMRUP EXPRESS' : rawTrainName;
    ...
    const distanceStr = booking.trainId?.distance ? `${booking.trainId.distance} KM` : (booking.distance ? `${booking.distance} KM` : 'N/A');
